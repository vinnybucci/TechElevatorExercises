  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      .then(promise => {
        ...
        {
          // let's grab the boards for display
          this.retrieveBoards();
          // turn off the add board form
          this.showAddBoard = false;
          // let's clear the add board form
          this.newBoard = {
            title: '',
            backgroundColor: this.randomBackgroundColor()
          }
      ...
      .catch(error => {
        // check if error is in request or response
        if(error.response) {
          this.errorMsg = "Error submitting new board. Server returned " + error.response.statusText + ".";
        } else if(error.request)
        {
          this.errorMsg = "Error submitting new board. Server returned  Can't reach the server.";
        } else {
          this.errorMsg = "Error submitting new board. Ya lost me.";
        }
      })
      this.isLoading = false;
    },
