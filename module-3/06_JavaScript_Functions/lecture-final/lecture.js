...
/* Work the examples from lecture */
function startsWithA()
{
  let wordsToFilter = ["Always","Anywhere","Bravo","Bully","about"];
  let wordsStartingWithA = wordsToFilter.filter((wordIn) => {
    return wordIn.startsWith("A") || wordIn.startsWith("a");
  })
  console.table(wordsStartingWithA);
}

let numbers = [1,2,3,4,5,6,7,8]
numbers.forEach((fred) => {
  console.log(`This number is ${fred}`)
});

function mapFromLecture()
{
  let numbersToSquare = [1,2,3,4,5,6];
  let squaredNumbers = numbersToSquare.map((number) => {
    return number * number + " <-- it's squared";
  })
...
function sumAllNumbers(numbersToSum) {
  return numbersToSum.reduce((sum,numberIn) => {
    return sum += numberIn;
  });
}
...
function allDivisibleByThreeWithFunctions(numbersToFilter,filterToUse)
{
  return numbersToFilter.filter(filterToUse);
}
