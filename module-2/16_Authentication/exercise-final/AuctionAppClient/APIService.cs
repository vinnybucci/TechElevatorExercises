...
namespace AuctionApp
{
    public class APIService
    {
        ...
        public void DeleteAuction(int auctionId)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed || !response.IsSuccessful)
            {
                throw new HttpRequestException(ProcessErrorResponse(response));
            }
        ...
        public API_User Login(string submittedName, string submittedPass)
        {
            ...
            if (response.ResponseStatus != ResponseStatus.Completed)
            {
                throw new HttpRequestException("An error occurred communicating with the server.");
            }
            else if (!response.IsSuccessful)
            {
                if (!string.IsNullOrWhiteSpace(response.Data.Message))
                {
                    throw new HttpRequestException("An error message was received: " + response.Data.Message);
                }
                else
                {
                    throw new HttpRequestException("An error response was received from the server. The status code is " + (int)response.StatusCode);
                }
            }
            else
            {
